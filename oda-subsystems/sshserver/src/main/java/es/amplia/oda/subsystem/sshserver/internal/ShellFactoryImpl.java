...
/**
 * SSHD {@link Command} factory which provides access to
 * Shell.
 */
class ShellFactoryImpl implements Factory<Command> {

    private static final Logger LOGGER = LoggerFactory.getLogger(ShellFactoryImpl.class);

    private final CommandProcessor processor;

    ShellFactoryImpl(CommandProcessor processor) {
    ...
    static void close(Closeable... closeables) {
        for (Closeable c : closeables) {
            try {
                ...
                LOGGER.warn("Error closing {}: {}", c, exception);
            }
        }
    }
    ...
    class ShellImpl implements Command, SessionAware {
        ...
        public void setSession(ServerSession session) {
            // Session is created from the Command Processor
        }
        ...
        void run(Environment env) {
            try (Terminal terminal = TerminalBuilder.builder().name("gogo").type(env.getEnv().get("TERM")).system(false).streams(in, out).build();
                ...
                setTerminalAttributesFromEnvironment(env, terminal);
        ...
        private void setTerminalAttributesFromEnvironment(Environment env, Terminal terminal) {
        ...
        private void flush(OutputStream... streams) {
            for (OutputStream s : streams) {
                try {
                    s.flush();
                } catch (IOException exception) {
                    LOGGER.warn("Error flushing the stream {}: {}", s, exception);
                }
            }
        }
    }
