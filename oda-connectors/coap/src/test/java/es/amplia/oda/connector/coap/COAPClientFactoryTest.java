...
@PrepareForTest(COAPClientFactory.class)
public class COAPClientFactoryTest {
    ...
    private static final ConnectorConfiguration TEST_OS_CONFIGURATION =
            ConnectorConfiguration.builder().remoteHost(TEST_REMOTE_HOST).remotePort(TEST_REMOTE_PORT).path(TEST_PATH)
    ...
    private static final ConnectorConfiguration TEST_AT_CONFIGURATION =
            ConnectorConfiguration.builder().type(ConnectorType.AT).remoteHost(TEST_REMOTE_HOST)
                    .remotePort(TEST_REMOTE_PORT).localPort(TEST_LOCAL_PORT).path(TEST_PATH)
                    .provisionPath(TEST_PROVISION_PATH).timeout(TEST_TIMEOUT)
                    .messageProtocolVersion(TEST_MESSAGE_PROTOCOL_VERSION).build();
    ...
    @Test
    public void testCreateOSClient() {
        CoapClient client = testCoapClientFactory.createClient(TEST_OS_CONFIGURATION);

        assertNotNull(client);
        String uri = client.getURI();
        assertTrue(uri.contains(COAP_SCHEME));
        assertTrue(uri.contains(TEST_REMOTE_HOST));
        assertTrue(uri.contains(String.valueOf(TEST_REMOTE_PORT)));
    ...
    public void testCreateATClient() throws Exception {
        ...
        assertTrue(uri.contains(TEST_PROVISION_PATH));
        assertEquals(TEST_TIMEOUT * MS_PER_SECOND, client.getTimeout());
        PowerMockito.verifyNew(ATUDPConnector.class).withArguments(eq(mockedATManager), eq(TEST_REMOTE_HOST), eq(TEST_REMOTE_PORT),
                eq(TEST_LOCAL_PORT));
    ...
    public void testCreateOptions() {
        ...
        OptionSet optionSet = testCoapClientFactory.createOptions(TEST_OS_CONFIGURATION);

        assertNotNull(optionSet);
        assertTrue(optionSet.getOthers().stream()
    ...
    public void testCreateOptionsNoDeviceId() {
        ...
        testCoapClientFactory.createOptions(TEST_OS_CONFIGURATION);
