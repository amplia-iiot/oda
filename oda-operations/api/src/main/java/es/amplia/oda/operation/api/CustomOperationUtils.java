...
public class CustomOperationUtils {
    ...
    private static Step translateStep(es.amplia.oda.core.commons.utils.operation.response.Step step) {
        return new Step(step.getName(), getStepResult(step.getResult()), step.getDescription(),
                step.getTimestamp() == null ? System.currentTimeMillis() : step.getTimestamp(), step.getResponse());
    }
