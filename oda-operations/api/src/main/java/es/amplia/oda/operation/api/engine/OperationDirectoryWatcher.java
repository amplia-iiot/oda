		...
		try {
			...
			creatingWatcherThread = new Thread(() -> {
				try {
					...
					log.error("Something unexpected happened while watching the directory for changes. Watcher will be stopped", e);
					Thread.currentThread().interrupt();
				}
			});
			creatingWatcherThread.start();
		} catch (IOException e) {
			log.error("Error on Operations directory Watcher creation", e);
		}
	}
