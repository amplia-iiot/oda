...
public class ModelsDirectoryWatcher implements DirectoryWatcher {

	private static final Logger LOGGER = LoggerFactory.getLogger(ModelsDirectoryWatcher.class);
	...
	public ModelsDirectoryWatcher(Path path, InferenceEngine engine) {
		this.path = path;
		this.engine = engine;
	}
					...
					while(true) {
						...
						key.pollEvents().forEach(event -> {
							if (event.kind().name().equals(ENTRY_CREATE.name())) {
								engine.importModel(this.path.toString() + FileSystems.getDefault().getSeparator() + event.context().toString());
							} else if (event.kind().name().equals(ENTRY_DELETE.name())) {
								engine.removeImportedModel(this.path.toString() + FileSystems.getDefault().getSeparator() + event.context().toString());
							}
						});
						key.reset();
					}
